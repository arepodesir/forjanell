...
export default function BubbleWishes() {
  const [bubbles, setBubbles] = createSignal(
    WISHES.map((wish, id) => ({
      ...
      wish,
      popped: false,
      x: Math.random() * 70 + 10, // percentage left
      y: Math.random() * 50 + 20, // percentage top
      size: Math.random() * 30 + 90, // size in pixels
  ...
  return (
    <div class="relative w-full flex flex-col items-center justify-center overflow-hidden py-4">
      {/* Instructions */}
      <div class="z-10 text-center mb-8 px-4">
        <h2 class="text-2xl font-bold text-white mb-2 drop-shadow-md" style={{ "font-family": "'Outfit', sans-serif" }}>
          Pop the Bubbles to Reveal Your Wishes! 🫧🎈
        </h2>
        <p class="text-blue-100/80 text-sm">
          Click on any floating bubble below to release its hidden blessing.
        </p>
      </div>

      {/* Floating Bubbles Stage */}
      <div class="relative w-full h-[55vh] max-w-4xl border border-white/10 rounded-3xl bg-white/5 backdrop-blur-md overflow-hidden p-6 shadow-2xl">
        <For each={bubbles()}>
          {(bubble) => (
        ...
        <Show when={activeWish()}>
          <div class="absolute inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-6 z-20">
            <div class="aero-glass rounded-3xl p-8 max-w-md w-full text-center border-2 border-aero-cyan shadow-aero-cyan/20 animate-reveal-up">
              <span class="text-5xl block mb-4">🎉</span>
              <p class="text-xl font-bold text-white leading-relaxed mb-6" style={{ "font-family": "'Outfit', sans-serif" }}>
                {activeWish()}
              </p>
              <button
                onClick={() => setActiveWish(null)}
                class="aero-btn text-xs uppercase tracking-wider px-6 py-2"
              >
                Keep Popping! 🫧
              </button>
            </div>
