import { c as createComponent$1, $ as $$Application } from './Application_Bbz4iz8D.mjs';
import 'piccolore';
import { r as renderComponent, a as renderTemplate, m as maybeRenderHead } from './prerender_vAft8ZjS.mjs';
import { ssr, ssrHydrationKey, escape, ssrStyleProperty, ssrAttribute, createComponent } from 'solid-js/web';
import { createSignal, onMount, onCleanup, createEffect, Show } from 'solid-js';
import { H as Hadacard } from './Hadacard_DCt-fqSt.mjs';
import { A as APP_CONFIG } from './config_CwuTGXRM.mjs';
import { B as BackButton } from './BackButton_Lu0ppxms.mjs';

var _tmpl$$2 = ["<div", ' class="', '" style="', '"><canvas class="absolute inset-0 w-full h-full"></canvas><!--$-->', "<!--/--><!--$-->", "<!--/--></div>"], _tmpl$2$1 = ["<div", ' class="absolute inset-0 flex items-center justify-center z-10 pointer-events-none"><div class="aero-glass px-5 py-2 rounded-full text-xs tracking-[3px] text-aero-cyan/90">blooming the orchid...</div></div>'], _tmpl$3$1 = ["<div", ' class="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 text-[10px] text-white/50 font-mono bg-black/40 px-3 py-1 rounded">', "</div>"];
function OrchidViewer(props) {
  ...
  return ssr(_tmpl$$2, ssrHydrationKey(), `relative w-full overflow-hidden rounded-3xl bg-[#0a0b18] ${escape(props.class || "h-[520px] md:h-[620px]", true)}`, ssrStyleProperty("touch-action:", "none"), isLoading() && _tmpl$2$1[0] + ssrHydrationKey() + _tmpl$2$1[1], error() && ssr(_tmpl$3$1, ssrHydrationKey(), escape(error())));
}
...
var _tmpl$ = ["<div", ' class="relative z-20 flex flex-col items-center justify-center text-center text-white p-6" style="', '"><h1 class="text-4xl font-extrabold mb-6 drop-shadow-lg" style="', `">Come Back Later...</h1><div class="aero-glass p-6 rounded-2xl max-w-md border border-white/10"><p class="text-xl">I know you're excited for your special day, but you need to be patient! ✨</p></div></div>`], _tmpl$2 = ["<div", ' class="relative z-20 flex flex-col items-center justify-center text-center text-white p-6" style="', '"><h1 class="text-4xl font-extrabold mb-6 drop-shadow-lg text-pink-300" style="', '">The Party is Over</h1><div class="aero-glass p-6 rounded-2xl max-w-md border border-white/10"><p class="text-xl">You missed it! But the gift was meant for you. Happy Belated Birthday! 💖</p></div></div>'], _tmpl$3 = ["<div", ' class="z-20 absolute inset-0 flex items-center justify-center select-none">', "</div>"], _tmpl$4 = ["<button", ' class="fixed bottom-8 left-1/2 -translate-x-1/2 z-[70] aero-btn text-xs tracking-wider px-6 py-2" style="', '">Open the Letter &amp; Meet Your Orchid 🌸</button>'], _tmpl$5 = ["<div", ' class="z-20 w-full flex flex-col items-center justify-start p-4 pt-8 pb-12 gap-5"><div class="aero-glass rounded-3xl px-5 py-4 max-w-md text-center border border-white/15"><p class="text-aero-cyan text-sm tracking-wider uppercase mb-1" style="', '">Your gift has opened</p><div class="text-white/90 text-[15px] leading-snug" style="', '">A living orchid, grown just for you. Touch it. Turn it. Let it catch the light.</div></div><!--$-->', '<!--/--><p class="text-[10px] text-white/40 tracking-[2px] mt-1">TAP THE ORCHID • DRAG TO ORBIT • PINCH TO ZOOM</p><!--$-->', "<!--/--></div>"], _tmpl$6 = ["<div", ' class="relative w-full h-full min-h-screen flex flex-col items-center justify-center overflow-hidden" style="', '"><canvas class="absolute inset-0 w-full h-full pointer-events-none z-0"></canvas><div class="absolute inset-0 bg-gradient-to-b from-blue-950/30 via-transparent to-purple-950/40 pointer-events-none z-10"></div><div class="', '" style="', '"></div><!--$-->', "<!--/--><!--$-->", "<!--/--><!--$-->", "<!--/--></div>"];
function FrutigerScenes(props) {
  ...
  onMount(() => {
    if (props.openDate) {
      ...
      const now2 = Date.now();
      if (now2 < startTime) setTimeStatus("EARLY");
      else if (now2 > endTime) setTimeStatus("LATE");
    }
  ...
  return ssr(_tmpl$6, ssrHydrationKey(), ssrStyleProperty("background:", "linear-gradient(135deg, #090c21 0%, #05060f 50%, #0d071a 100%)"), `absolute inset-0 z-40 pointer-events-none bg-white/5 transition-all duration-[1600ms] ease-out ${isTransitioning() ? "opacity-90" : "opacity-0"}`, ssrStyleProperty("backdrop-filter:", isTransitioning() ? "blur(28px) saturate(0.7)" : "blur(0px) saturate(1)") + ssrStyleProperty(";transition:", "backdrop-filter 1600ms cubic-bezier(0.23,1,0.32,1), opacity 900ms ease"), escape(createComponent(Show, {
    get when() {
    ...
    get children() {
      return [createComponent(Show, {
        ...
        get children() {
          ...
          }))), ssr(_tmpl$4, ssrHydrationKey(), ssrStyleProperty("font-family:", "'Comfortaa', sans-serif"))];
        }
        ...
        get children() {
          return ssr(_tmpl$5, ssrHydrationKey(), ssrStyleProperty("font-family:", "'Comfortaa', sans-serif"), ssrStyleProperty("font-family:", "'Patrick Hand', cursive"), escape(createComponent(OrchidViewer, {
            "client:load": true,
